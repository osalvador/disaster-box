DROP PACKAGE BODY TAPIGEN.TAPI_GEN2
/

CREATE OR REPLACE PACKAGE BODY TAPIGEN.tapi_gen2
AS
   --Global private variables
   g_unque_key   dbo_name_t;

   function create_tapi_package (p_table_name               IN VARCHAR2
                                , p_compile_table_api        IN BOOLEAN DEFAULT FALSE
                                ...
                                , p_user_date                IN DATE DEFAULT SYSDATE) return clob
   AS
   ...
   BEGIN
      ...
      l_vars ('date') := to_char (p_user_date, 'YYYY-MM-DD');
      ...
      return l_spec_tapi || CHR(10) || '/' || l_body_tapi; 
   ...
   FUNCTION create_tapi_package (p_table_name               IN VARCHAR2
                               ...
                               , p_modified_date_col_name   IN VARCHAR2 DEFAULT NULL
                               , p_raise_exceptions         IN BOOLEAN DEFAULT FALSE
                               , p_user_date                IN DATE DEFAULT SYSDATE )
      RETURN CLOB
   AS
      l_count        PLS_INTEGER := 0;
      l_table_name   dbo_name_t := lower (p_table_name);
   ...
   BEGIN
      ...
      --Init variables for render template
      l_vars ('date') := to_char (p_user_date, 'DD-MM-YYYY');
      l_vars ('table_name') := l_table_name;
      l_vars ('user') := user;
      l_vars ('created_by_col_name') := p_created_by_col_name;
      l_vars ('created_date_col_name') := p_created_date_col_name;
      ...
      l_return := l_spec_tapi || '/' || CHR(10) || l_body_tapi || '/' ; 
